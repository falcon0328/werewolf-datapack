...
# タイマーの名前をN日目 昼の形式に変更する
# 通信切断などの対策のために随時表示名更新を行なっている
bossbar set day_timer_bossbar name [{"score":{"name":"@e[type=armor_stand,nbt={Tags: [\"game_manager\"]},limit=1]","objective":"day_count"},"bold":true},{"text":"日目","bold":true},{"text":" 昼"}]
